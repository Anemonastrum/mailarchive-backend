...
export const getAllDocumentsWait = async (req, res) => {
  try {
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 10;
    const search = req.query.search || '';
    ...
    const inboxQuery = {
      $and: [
        ...
        { status: 'wait' } // Only include inboxes that are marked as done
      ]
    };
    ...
    const [inboxes, outboxes] = await Promise.all([
      Inbox.find(inboxQuery)
        .select('number status date summary origin attachment createdAt')
        .sort({ createdAt: -1 }),
      Outbox.find(outboxQuery)
        .select('number status date summary destination attachment createdAt')
        .sort({ createdAt: -1 }),
    ]);
